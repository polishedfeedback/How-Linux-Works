...
### Harddisks: /dev/sd*
- Most devices in Linux start with `sd` prefix
- Kernel makes separate device files for each partitions on a disk
- The `sd` portion stands for *SCSI* which means *Small Computer System Interface* which was developed as a hardware and protocol standard for communication between devices such as disks and other peripherals
- Linux still uses *SCSI* communication although it isn't used in most modern machines
- `lssci` can be used to see the list of device paths provided by `sysfs` 
- 1st column in the above command's output tells you the address of the device, 2nd column describes what type of disk it is and the last column describes the path to the device
- Newer hardware called NVMe do not use *SCSI* layer so they show up as `/dev/nvme01` and etc
- Linux, as described, names these devices in the order that they are found. So it is harder to just use the device with the name of the device. For example, if you unmount a device, `/dev/sdb` becomes `/dev/sda` which is why linux uses `uuid` to give IDs to these devices

### Virtual Devices
- Some disk devices are optimised for virtual machines such as AWS instances and VirtualBox. 
- They are prefixed with `/dev/xvd` or `/dev/vd` 

### Non-Volatile Memory Devices: `/dev/nvme`
- Some systems now use Non-Volatile Memory Expresss (NVMe) interface to talk to some kinds of solid-state storage. 
- In Linux, these devices show up at `/dev/nvme*` and can be viewed with `nvme list` command
---
